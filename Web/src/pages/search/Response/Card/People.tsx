                ...
                props.results.results && props.results.results.map((result: any, index: Key) => (
                    <Grid item xs={12} sm={6} md={4} lg={3} key={index}>
                        <Card style={{padding: 16, width: "100%"}}>
                            <Grid container spacing={2} rowSpacing={2}>
                                <Grid item xs={12}>
                                    <Avatar alt={result.name} src={`https://starwars-visualguide.com/assets/img/${result.url.replace(baseURL, '').replace('people', 'characters')}.jpg`} sx={{ width: 100, height: 100 }} />
                                ...
                                <Grid item xs={12}>
                                    <Typography gutterBottom>
                                        Taille : {result.height !== 'unknown' ? (result.height + ' cm') : 'Inconnu'}
                                    </Typography>
                                </Grid>
                                <Grid item xs={12}>
                                    <Typography gutterBottom>
                                        Poids : {result.mass !== 'unknown' ? (result.mass + ' kg') : 'Inconnu'}
                                    </Typography>
                                </Grid>
                                <Grid item xs={12}>
